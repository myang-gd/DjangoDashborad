		...
		for(var j=0; j < data.length; j++){							
			...
			if(data[j]["custType"] == custtypes[i] && data[j]["isVisible"] == "True" && data[j]["isEnable"] == "True"){								
				...
				if(data[j]["Params"] != '' && data[j]["Params"] != null){
					...
					}else if(data[j]["custType"] == 'MemberShipGroupKey'){
						content = content + '<input type="text" id="p_' + data[j]["Params"] +'" value="=113" placeholder="' + data[j]["Params"] + '" data-toggle="tooltip" title="Param MemberShipGroupKey Format like: =113, in (113, 115) "></input>';					
					}else{
